...
namespace SML.Common
{
    public class SegmentTreeAllocator
    {
        ...
        public int Rent(int size, AllocType allocType)
        {
            switch (allocType)
            {
                case AllocType.FirstFit:
                    return RentFirstFit(0, 0, capacity, size);
                case AllocType.BestFit:
                    return RentBestFit(0, 0, capacity, size);
                case AllocType.WorstFit:
                    return RentWorstFit(0, 0, capacity, size);
                default:
                    return -1;
            }
        }
        ...
        private int RentBestFit(int node, int start, int end, int size)
        {
            ...
            {
                return tree[2 * node + 1] < tree[2 * node + 2] ? leftResult : rightResult;
            }
